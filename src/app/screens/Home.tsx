...
function cardBadge(s: CardStatus) {
  if (s === 'Overdue') return 'bg-red-100 dark:bg-red-500/15 text-red-600 dark:text-red-400';
  if (s === 'Draft') return 'bg-secondary text-muted-foreground';
  return 'bg-blue-100 dark:bg-blue-500/15 text-primary-blue';
}
...
export default function Home() {
  ...
  return (
    <div className="bg-surface min-h-full">
      ...
      {/* ── Top App Bar ───────────────────────────────────────── */}
      <header className="sticky top-0 z-40 bg-card/95 backdrop-blur-sm border-b border-divider/70 px-4 status-bar-aware">
        <div className="flex items-center justify-between pb-3">
          <div>
            <p className="text-[16px] font-bold text-foreground leading-tight">G Tech Auditor</p>
            <p className="text-[11px] text-muted-foreground mt-0.5">{todayLabel}</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => navigate('/app/notifications')}
              className="relative w-9 h-9 rounded-xl border border-divider bg-card flex items-center justify-center"
              aria-label="Notifikasi"
            >
              <Bell className="w-4.5 h-4.5 text-foreground" strokeWidth={1.8} />
      ...
      <div className="px-4 pt-4 pb-6 space-y-4">
        ...
        <section className="rounded-2xl border border-divider bg-card overflow-hidden">
          <div className="flex">
            ...
            <div className="flex-1 px-4 pt-4 pb-3">
              ...
              <div className="space-y-0 divide-y divide-divider/50">
                {TOP_DEALERS.map((dealer, i) => (
                  <div key={dealer.name} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
                    <span className="w-5 text-[12px] font-bold text-muted-foreground flex-shrink-0 text-right">{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] font-semibold text-foreground truncate leading-tight">{dealer.name}</p>
        ...
        <div className="relative -mx-4">
          <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-10 bg-gradient-to-l from-surface to-transparent z-10" />
          <div className="overflow-x-auto scrollbar-hide">
            <div className="flex items-center gap-2 pl-4 pr-10 min-w-max py-0.5">
              <div className="h-8 rounded-full px-3.5 bg-emerald-50 dark:bg-emerald-500/15 text-[12px] text-emerald-600 dark:text-emerald-400 font-medium inline-flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5" strokeWidth={2} />
                {stats.selesai} Selesai
              </div>
              <div className="h-8 rounded-full px-3.5 bg-blue-50 dark:bg-blue-500/15 text-[12px] text-primary-blue font-medium inline-flex items-center gap-1.5">
                <Layers className="w-3.5 h-3.5" strokeWidth={2} />
                {stats.aktif} Aktif
              </div>
              <div className="h-8 rounded-full px-3.5 bg-secondary text-[12px] text-muted-foreground font-medium inline-flex items-center gap-1.5">
                <Clock3 className="w-3.5 h-3.5" strokeWidth={2} />
                {stats.draft} Draft
              </div>
              <div className="h-8 rounded-full px-3.5 bg-red-50 dark:bg-red-500/15 text-[12px] text-danger-red dark:text-red-400 font-medium inline-flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          ...
          <button
            ...
            className="h-12 rounded-xl bg-card border border-divider text-[14px] font-semibold text-foreground active:scale-[0.98] transition-transform"
          >
            Pelatihan Tim
          </button>
        </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <p className="text-[15px] font-bold text-foreground">Sedang Berjalan</p>
              ...
              <div className="flex gap-3 pl-4 pr-10 min-w-max pb-1">
                {IN_PROGRESS_CARDS.map(card => (
                  <button
                    ...
                    className="w-[220px] flex-shrink-0 rounded-2xl bg-card border border-divider overflow-hidden text-left active:scale-[0.98] transition-transform shadow-sm"
                  >
                    {/* Top accent bar */}
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-[14px] font-bold text-foreground leading-snug line-clamp-1">{card.title}</p>
                      <p className="text-[11px] text-muted-foreground mt-1 leading-snug line-clamp-2">{card.subtitle}</p>
                      <div className="inline-flex items-center gap-1.5 mt-3 px-2 py-1 rounded-lg bg-surface border border-divider/50 text-muted-foreground">
                        <Clock3 className="w-3 h-3 text-muted-foreground" strokeWidth={2} />
                ...
                <button
                  onClick={() => navigate('/app/inspections')}
                  className="w-[120px] flex-shrink-0 rounded-2xl border-2 border-dashed border-divider bg-card flex flex-col items-center justify-center gap-2 text-muted-foreground active:scale-[0.98] transition-transform"
                  style={{ minHeight: 140 }}
                >
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <p className="text-[15px] font-bold text-foreground">Agenda</p>
          </div>
              ...
              <button
                ...
                className={`h-8 px-4 rounded-full text-[12px] font-semibold whitespace-nowrap flex-shrink-0 transition-colors ${
                  agendaTab === tab
                    ...
                    : 'bg-card border border-divider text-muted-foreground'
                }`}
              >
                {tab}
              </button>
          ...
          {/* Empty state */}
          <div className="rounded-2xl bg-card border border-divider py-10 px-6 flex flex-col items-center text-center">
            <div className="w-14 h-14 rounded-2xl bg-surface flex items-center justify-center mb-4">
              <CalendarDays className="w-7 h-7 text-muted-foreground" strokeWidth={1.5} />
            </div>
            <p className="text-[14px] font-semibold text-foreground">Semua beres!</p>
            <p className="text-[12px] text-muted-foreground mt-1.5 leading-relaxed max-w-[220px]">
